...
class EnhancedRolePermissions:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.role_permissions = {
            ...
            'member': {
                ...
                # Kanban Board Operations (limited)
                Permission.CREATE_CARD,  # Only in assigned projects
                Permission.UPDATE_OWN_CARD,
                Permission.DELETE_OWN_CARD,
    ...
    async def _check_resource_permission(
        ...
        # For card operations, check if user is assigned to the card
        if permission in [Permission.UPDATE_OWN_CARD, Permission.DELETE_OWN_CARD]:
            result = await self.db.execute(
                select(CardAssignment)
                .where(
                    and_(
                        CardAssignment.card_id == resource_id,
                        CardAssignment.user_id == user_id
                    )
                )
            )
